...
function makeMainVerbTask() {
  ...
  let choices = [
    ...
    },
    {
      text: wrongWould,
      isCorrect: false,
      wrongReason: "Would gehört nicht zur grundlegenden Part-1-Form.",
    },
    {
      text: wrongPast,
      isCorrect: false,
...
function makeFullSentenceTask() {
  ...
  return {
    type: "fullSentence",
    prompt: "Wähle den grammatisch korrekten Satz.",
    choices,
...
function checkDrillRound() {
    ...
    if (selectedChoice?.isCorrect) {
      ...
      feedback.innerHTML = `
        <p class="feedback bad">
          Nicht korrekt. ${selectedChoice?.wrongReason || "Prüfe die Struktur erneut."}
          Richtige Lösung: "${correctChoice?.text || ""}".
        </p>
      `;
    }
  });
  ...
  enDrillFeedback.innerHTML = `
    <p class="feedback ${allCorrect ? "ok" : "bad"}">
      ...
      ${allCorrect ? "Sehr stark." : "Nutze die Erklärungen bei den roten Aufgaben und starte danach eine neue Runde."}
    </p>
  `;
}
...
function checkDetectiveRound() {
  ...
  if (answered < detectiveTasks.length) {
    enDetectiveFeedback.innerHTML = '<p class="feedback info">Bitte alle Sätze markieren.</p>';
    return;
  }
...
function fixedQuizQuestions() {
  return [
    {
      prompt: "Welche Struktur passt für Part 1 am besten?",
      options: [
      ...
      correct: 0,
      explanation: "Diese Form wird für reale/mögliche Folgen verwendet.",
      wrongReasons: {
        1: "Will steht in Part 1 nicht im if-Teil.",
        2: "Past simple gehört nicht zur Grundform von Part 1.",
    ...
    {
      prompt: "Welche Aussage zu if-Sätzen in Part 1 ist richtig?",
      options: [
        "Im if-Teil steht immer will.",
        "Die Reihenfolge der Teilsätze kann variieren.",
        "Ein if-Satz hat nur einen Teilsatz.",
        "Im Hauptsatz steht nie will.",
      ],
      correct: 1,
      explanation: "If-Clause und main clause können in beiden Reihenfolgen stehen.",
      wrongReasons: {
        0: "Das ist genau eine typische Fehlerfalle.",
        2: "If-Sätze bestehen aus Bedingung und Folge.",
    ...
    },
    {
      ...
      wrongReasons: {
        ...
        2: "Hier wird kein past tense benötigt.",
...
function finishQuiz() {
  ...
  enQuizFeedback.innerHTML = '<p class="feedback info">Nutze die Fehlererklaerungen als Lernhilfe für die nächste Runde.</p>';
  enQuizNext.disabled = true;
...
function submitQuizAnswer(optionIndex) {
  ...
  const wrongReason = question.wrongReasons?.[optionIndex] || "Prüfe die Grundregel: if + present simple, will + infinitive.";
  enQuizFeedback.innerHTML = `
